...
public Configuration loadConfiguration(){
    ...
    str defaultInput = "/data/project";
    list[str] defaultLibs = ["/data/ext/msvc"];

    try {
        println("Loading config file from <configurationLoc>...");

        map[str, value] jsonMap = readJSON(#map[str, value], configurationLoc);

        list[str] libs = [];

        if (str libStr := jsonMap["externalLibRoot"]) {
            libs = split(";", libStr);
        }
        ...
        return configuration(
            jsonMap["inputFolderAbsolutePath"] ? defaultInput,
            libs,
            jsonMap["saveFilesAsJson"] ? true,
            jsonMap["composeModels"] ? true,
            jsonMap["verbose"] ? false,
            jsonMap["localDrive"] ? "C:",
            jsonMap["saveUnresolvedIncludes"] ? false
        );
    } catch IO(msg): {
